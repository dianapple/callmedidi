...
class PxColor
{
    public static function Px_Desaturate($value, $amount, $min=0)
    {
        //Only support hex for now
        if(!self::Px_IsHex($value))
            return '';

        $color = self::Px_HexToHsv($value);

        $color[1] = max($color[1] - $amount, $min);

        $color = self::Px_HsvToHex($color[0], $color[1], $color[2]);
        return $color;
    }

    public static function Px_IsHex($value)
    {
        return preg_match("/^#([A-Fa-f0-9]{6}|[A-Fa-f0-9]{3})$/", $value) === 1;
    }

    public static function Px_IsColor($value)
    {
        $value = trim($value);
        if(self::Px_IsHex($value))
            return true;

        if(self::Px_NamedColorToHex($value))
            return true;

        return false;
    }

    public static function Px_HexToHsv($value)
    {
        $rgb = self::Px_HexToRgb($value);

        return self::Px_RgbToHsv($rgb[0], $rgb[1], $rgb[2]);
    }

    public static function Px_RgbToHsv ($R, $G, $B)  // RGB Values:Number 0-255
    ...
    public static function Px_HsvToRgb($iH, $iS, $iV)
    {
    ...
    public static function Px_HsvToHex($iH, $iS, $iV)
    {
        $rgb = self::Px_HsvToRgb($iH, $iS, $iV);
        return self::Px_RgbToHex($rgb[0], $rgb[1], $rgb[2]);
    }

    //Converts hex color value to array of RGB
    public static function Px_HexToRgb($color)
    {
        //Modified version of:
        //http://bavotasan.com/2011/convert-hex-color-to-rgb-using-php/

        //Validate the input
        if(!self::Px_IsHex($color))
            return array(0,0,0);
    ...
    public static function Px_RgbToHex($r, $g, $b) {
    ...
    public static function Px_NamedColorToHex($name, $retArray=false)
    {
        $colors = array(
