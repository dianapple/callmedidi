...
class PxTwitterTimeline
{

    public function Px_TheTimeline(array $settings)
    {
        $defaults = array(
            'consumer_key' => false,
        ...
        if($tmhOAuth->response['errno'] != 0 || $tmhOAuth->response['error'] != '' )
        {
            ...
            return $this->Px_TimelineOutput($settings, $output);
        }
        ...
        //Get user timeline
        $scrName = ( isset($userSettings->screen_name) && !is_null($userSettings->screen_name) )? $userSettings->screen_name : "";
        $code = $tmhOAuth->request('GET', $tmhOAuth->url('1.1/statuses/user_timeline'), array(
            'screen_name' => $scrName , 'count' => $settings['count']));
        ...
        foreach($timeline as $item)
            $output .= $this->Px_FormatTimeline($item);
	...
	protected function Px_TimelineOutput(array $settings, $output)
	{
		if($settings['echo']) 
			echo $output; 
		else
    ...
    protected function Px_FormatTimeline($item)
    {
        $status = $this->Px_ConvertReplies( $this->Px_ConvertUrls($item->text) );

        return '<li><span>' . $status . '</span><br/><a class="link" href="http://twitter.com/' . $item->user->screen_name . '/statuses/' . $item->id_str . '">' . $this->Px_RelativeTime($item->created_at) . '</a></li>';
    }
	
	protected function Px_ConvertUrls($status)
	{
		return preg_replace('/((https?|s?ftp|ssh)\:\/\/[^"\s\<\>]*[^.,;\'">\:\s\<\>\)\]\!])/', '<a href="$1">$1</a>', $status);
	}

	protected function Px_ConvertReplies($status)
	{
		return preg_replace_callback('/\B@([_a-z0-9]+)/i', array( &$this, 'PxReplyRegex_Callback'), $status);
	}

	protected function PxReplyRegex_Callback($matches)
	{
		return $matches[0]{0} . '<a href="http://twitter.com/'. $matches[1] .'">'. $matches[1] .'</a>';
	}

	protected function Px_RelativeTime($a)
	{
