...
class PixFlow_Testimonials_Widget extends WP_Widget {
	...
	public function __construct() {
		parent::__construct(
	 		'Px_Testimonials', // Base ID
			'Px - Testimonials Widgett', // Name
    ...
    function load_admin_scripts($hook)
    {
        ...
        wp_enqueue_script('widget-testimonials', px_path_combine(THEME_LIB_URI, 'widgets','js', 'widget-testimonials.js'), array('jquery'));
        wp_enqueue_style('widget-testimonials', px_path_combine(THEME_LIB_URI, 'widgets','css', 'widget-testimonials.css'));
    }
        ...
        <div class="testimonials-container">
            ...
            <ul>
                <?php for($i=0; $i<$instance['count']; $i++){ ?>
                    <li>
                        <h4 class="name"><?php if(function_exists('icl_translate')){
                                $translated = icl_translate(TEXTDOMAIN, 'testimonials_widget_text_' . $i, $instance["name$i"]);
                                echo $translated;
                            } else {
                                echo $instance["name$i"];
                            } ?></h4>
                        <blockquote><?php if(function_exists('icl_translate')){
                                $translated = icl_translate(TEXTDOMAIN, 'testimonials_widget_text_' . $i, $instance["text$i"]);
                                echo $translated;
                            } else {
                                echo $instance["text$i"];
                            } ?></blockquote>
                    </li>
                <?php
                }?>
