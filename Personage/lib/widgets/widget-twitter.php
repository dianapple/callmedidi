...
class PixFlow_Twitter_Widget extends WP_Widget {
    ...
    function widget( $args, $instance ) {
        ...
        <ul class="twitter-recent-list">
            ...
            $twitter = new PxTwitterTimeline();
            $cache   = new PxFileCache(array('directory'=> px_path_combine(THEME_CACHE , 'twitter'), 'cache_time'=>60));

            if( ($timeline = $cache->Px_GetCache($scrName)) != false)
            {
                echo $timeline;
            }
            else
            {
                $timeline = $twitter->Px_TheTimeline(array(
                ...
                $cache->Px_Save($scrName, $timeline);
                echo $timeline;
